...
import { removeItemFromOwnedList, isItemOwned } from '../components/OwnedItemList.js';
...
export function createItemCard(item) {
    ...
    card.innerHTML = `
        ...
        <div class="item-actions">
            ...
            <label class="fav-btn-label" style="margin-right:10px; cursor:pointer;">
                ...
                <div class="fav-btn-action">
                    <span class="fav-btn-option-1">${t('translations.SendToDesktop')}</span>
                    <span class="fav-btn-option-2">${t('translations.RemoveFromDesktop')}</span>
                </div>
            </label>
    ...
    favLabel.addEventListener('click', async (e) => {
        ...
        if (isFav) {
            favLabel.classList.remove('sent');
            await sendToDesktop(favLabel, item.id);
        } else {
